  ...
  Future<void> _openDocument(VaultFileData file) async {
    try {
      if (kIsWeb) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Membuka dokumen belum didukung di versi Web')),
          );
        }
        return;
      }
...
class _FullScreenVideoViewerState extends State<_FullScreenVideoViewer> {
  ...
  Future<void> _initVideo() async {
    try {
      if (kIsWeb) {
        // VideoPlayerController.file is not supported on web. 
        // For a full web implementation, Blob URLs should be used.
